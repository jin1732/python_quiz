...
- class Quiz: : 개별 퀴즈 데이터(질문, 보기, 정답)를 담아둘 붕어빵 틀 만들기(하나의 퀴즈와 정답 확인 기능을 담당)
- class QuizGame: : 게임 전체 흐름을 관리하는 틀 만들기(퀴즈 목록과 점수, 게임 진행 및 파일 저장을 담당)
...
- **클래스와 함수의 차이** 
  - 함수는 특정 기능을 수행하도록 코드를 묶어 재사용하는 데 사용한다.(함수만 사용하면 데이터를 함수마다 전달)
  - 클래스는 관련된 데이터(속성)와 기능(메서드)을 하나의 객체로 묶어 관리한다.
...
- **JSON을 사용하는 이유** : 퀴즈와 점수 데이터를 코드와 분리하여 저장하면 프로그램을 수정하지 않고도 데이터를 추가하거나 변경할 수 있기 때문에 JSON을 사용하였다.
...
### ⑧ 점수 저장 및 기록 확인 기능 구현
- 점수 저장 기능 : `datetime` 모듈을 사용하여 퀴즈 종료 후 최종 점수와 현재 날짜·시간을 함께 저장.
- 데이터 누적 : 기존 점수 기록을 불러와 새로운 점수를 추가한 후 다시 저장하는 방식으로 기록을 유지.
- 역대 기록 조회 : 저장된 점수와 날짜를 불러와 화면에 출력.
- 데이터 저장 구조 개선 : 초기에는 scores.json에 점수 데이터만 별도로 저장·관리하였으나, 이후 기능을 통합하면서 퀴즈 데이터(quizzes)와 점수 데이터(scores)를 하나의 state.json 파일로 통합하여 관리.
...
#### state.json 데이터 구조
```zsh
`state.json`은 프로젝트 루트에 위치하며 퀴즈 데이터와 점수 기록을 하나의 JSON 파일에서 관리한다.
